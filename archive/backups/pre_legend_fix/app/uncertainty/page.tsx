...
import { medicalDiagnosis as localMedical, sensorFusion as localSensor } from '@/lib/algorithms/bayes'
...
export default function UncertaintyPage() {
  ...
  const [sensors, setSensors] = useState<SensorInput[]>([
    { name: 'Barometer', reading: 0.85, reliability: 0.90 },
    { name: 'Thermometer', reading: 0.78, reliability: 0.85 },
    { name: 'Satellite', reading: 0.92, reliability: 0.95 },
  ])
  ...
  const runMedical = async () => {
    ...
    setError(null)
    try {
      const { data, engine: eng } = await fetchWithFallback<BayesResult>(
        '/api/uncertainty/execute',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ mode: 'medical', prevalence, sensitivity, fpr, testResult })
        },
        () => localMedical('Disease X', prevalence, sensitivity, fpr, testResult)
      )
      setEngine(eng)
      setBayesResult(data)
    } catch (err) {
      setError('Failed to run Bayesian analysis')
      console.error(err)
    } finally {
      setIsLoading(false)
    }
  ...
  const runSensor = async () => {
    ...
    setError(null)
    try {
      const { data, engine: eng } = await fetchWithFallback<{ combined: number; confidence: number; steps: BayesStep[] }>(
        '/api/uncertainty/execute',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      setError('Failed to run sensor fusion')
      console.error(err)
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="min-h-screen bg-surface-0 text-white">
      <CommandBar module="Uncertainty Intelligence Lab" engineIndicator={engine} />

      <div className="flex pt-14 h-screen">
        {/* Left Panel */}
        <aside className="w-64 bg-surface-1 border-r border-[rgba(255,255,255,0.06)] p-5 flex flex-col gap-5 overflow-y-auto flex-shrink-0">
          <div>
            <p className="text-[10px] text-text-tertiary uppercase tracking-wider mb-3">Scenario</p>
            {(['medical','sensor'] as Mode[]).map(m => (
              <button key={m} onClick={() => setMode(m)}
                className={`w-full text-left px-3 py-2.5 rounded-xl text-sm mb-1.5 transition-all duration-200 ${mode === m ? 'bg-surface-3 border border-[rgba(255,255,255,0.14)] text-white' : 'bg-surface-1 border border-transparent text-text-secondary hover:bg-surface-2'}`}>
                {m === 'medical' ? '🩺 Medical Diagnosis' : '🌤 Weather Prediction'}
              </button>
            ))}
          </div>
          ...
          {mode === 'medical' ? (
            <div className="space-y-4">
              ...
              <button onClick={runMedical} disabled={isLoading}
                className="bg-white text-black font-medium text-sm py-2.5 rounded-xl hover:bg-neutral-200 transition-all w-full disabled:opacity-50">
            ...
            <div className="space-y-4">
              ...
              <button onClick={runSensor} disabled={isLoading}
                className="bg-white text-black font-medium text-sm py-2.5 rounded-xl hover:bg-neutral-200 transition-all w-full disabled:opacity-50">
                {isLoading ? 'Computing...' : 'Predict Weather'}
              </button>
            </div>
          )}
        </aside>
        ...
        <main className="flex-1 flex flex-col min-w-0">
          <div className="flex-1 p-8 flex items-center justify-center overflow-y-auto">
            {mode === 'medical' ? (
              ...
              <div className="w-full max-w-3xl space-y-8">
                <p className="text-xs text-text-tertiary uppercase tracking-wider text-center">Weather Prediction Fusion</p>
                {sensorResult ? (
                  <div className="space-y-6">
                    ...
                    <div className="bg-surface-1 border border-[rgba(255,255,255,0.06)] rounded-2xl p-6 text-center">
                      <p className="text-xs text-text-tertiary uppercase tracking-wider mb-3">Predicted Rain Probability</p>
                  ...
                  <div className="text-center py-20 text-text-muted text-sm">Configure weather instruments and click Predict to see fusion</div>
                )}
              </div>
            )}
          </div>
          ...
          {/* Bottom: Reasoning */}
          <div className="h-44 border-t border-[rgba(255,255,255,0.06)] bg-surface-1 p-5 overflow-x-auto flex-shrink-0">
