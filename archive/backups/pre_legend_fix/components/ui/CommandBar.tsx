...
interface CommandBarProps {
  ...
  engineIndicator?: 'python' | 'local' | null
}
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 h-14 bg-surface-0/80 backdrop-blur-xl border-b border-subtle">
      <div className="flex items-center justify-between h-full px-6 max-w-screen-2xl mx-auto">
        ...
        {/* Center: Module Name + Mode Toggle + Children */}
        ...
        {/* Right: Engine Indicator + Back to Home */}
        <div className="flex items-center gap-4">
          {engineIndicator && (
            <div className="flex items-center gap-1.5">
              <span
                className={`inline-block w-1.5 h-1.5 rounded-full engine-indicator ${
                  engineIndicator === 'python'
                    ? 'bg-accent-green'
                    : 'bg-accent-amber'
                }`}
              />
              ...
              >
                {engineIndicator === 'python' ? 'Python Engine' : 'Local Engine'}
              </span>
            </div>
          )}
          <Link
